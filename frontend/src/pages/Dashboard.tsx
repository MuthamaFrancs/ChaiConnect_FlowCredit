...
import { fetchChartActivity, fetchRecentPayments, fetchStats, fetchFarmers, fetchAlerts, fetchFactoryLeaderboard } from '../lib/api'
...
export function DashboardPage() {
  ...
  const [eligibleFarmers, setEligibleFarmers] = useState<{ id: string; name: string; memberNo: string; creditScore: number }[]>([])
  ...
  useEffect(() => {
    void (async () => {
      ...
      setPayments(p)
      setChart(c)
      setEligibleFarmers((f as any[]).filter((x: any) => x.loanFlow === 'eligible').slice(0, 4))
      setAlerts(a)
      setLeaderboard(lb)
      setLoading(false)
    })()
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0,1fr))', gap: 14, marginTop: 18 }}>
        ...
        <StatCard label="Disbursed (month)" value={stats.disbursedMonth}
          format={(n) => { const m = n >= 1_000_000 ? `${(n / 1_000_000).toFixed(1)}M` : `${Math.round(n / 1000)}k`; return `KSh ${m}` }}
          gold />
        ...
        <div className="card-surface" style={{ padding: 0, overflow: 'hidden' }}>
          ...
          <table className="table-zebra" style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead><tr style={{ textAlign: 'left', fontSize: 12, color: 'var(--muted)' }}>
              <th style={{ padding: 12 }}>Farmer</th><th>Phone</th><th>Amount</th><th>Deductions</th><th>Net</th><th>Status</th><th>Time</th>
            </tr></thead>
            <tbody>
              {payments.map((p) => (
                <tr key={p.id}>
